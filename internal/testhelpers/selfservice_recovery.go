...
func GetVerificationFlow(t *testing.T, client *http.Client, isAPI bool, ts *httptest.Server) *kratos.SelfServiceVerificationFlow {
	...
	var rs kratos.SelfServiceVerificationFlow
	if isAPI {
		require.NoError(t, json.NewDecoder(res.Body).Decode(&rs))
	} else {
		f, _, err := publicClient.V0alpha2Api.GetSelfServiceVerificationFlow(context.Background()).Id(res.Request.URL.Query().Get("flow")).Execute()
