...
func (e *Verifier) ExecuteSettingsPostPersistHook(w http.ResponseWriter, r *http.Request, a *settings.Flow, i *identity.Identity, _ *session.Session) error {
	return e.do(r, i, a)
}
...
func (e *Verifier) do(r *http.Request, i *identity.Identity, f flow.Flow) error {
	...
	for k := range i.VerifiableAddresses {
		...
		if err := strategy.SendVerification(r.Context(), verificationFlow, i, address, body.TransientPayload, body.Branding); err != nil {
			return err
		}
	}
	return nil
