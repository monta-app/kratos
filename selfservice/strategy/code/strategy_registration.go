...
func (s *Strategy) registrationVerifyCode(ctx context.Context, f *registration.Flow, p *updateRegistrationFlowWithCodeMethod, i *identity.Identity) (err error) {
	...
	if errors.Is(err, ErrCodeNotFound) {
		...
		if !s.deps.Config().SelfServiceCodeStrategy(ctx).ExternalSMSVerify.Enabled ||
			registrationCode.AddressType != identity.AddressTypePhone {
			return errors.WithStack(errors.New("External SMS verify disabled or unexpected address type: " + registrationCode.AddressType))
		}
