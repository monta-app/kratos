...
func (v *ExternalVerifier) VerificationStart(ctx context.Context, t courier.SMSTemplate) (err error) {
	...
	builder, err := request.NewBuilder(v.deps.Config().SelfServiceCodeExternalSMSVerifyVerificationStartRequest(), v.deps)
	if err != nil {
	...
	if res.StatusCode >= 200 && res.StatusCode < 300 {
		...
		templateType, err := courier.SMSTemplateType(t)
		if err != nil {
			return errors.WithStack(err)
		}
		v.deps.Logger().
			WithField("to", phoneNumber).
			WithField("template_type", templateType).
...
func (v *ExternalVerifier) VerificationCheck(ctx context.Context, t courier.SMSTemplate) (err error) {
	...
	builder, err := request.NewBuilder(v.deps.Config().SelfServiceCodeExternalSMSVerifyVerificationCheckRequest(), v.deps)
	if err != nil {
		return errors.WithStack(err)
	}
