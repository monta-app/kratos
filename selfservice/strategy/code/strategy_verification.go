...
func (s *Strategy) verificationHandleFormSubmission(w http.ResponseWriter, r *http.Request, f *verification.Flow, body *updateVerificationFlowWithCodeMethodBody) error {
	...
	if err := s.deps.CodeSender().SendVerificationCode(r.Context(), f, via, to, body.TransientPayload); err != nil {
...
func (s *Strategy) SendVerification(ctx context.Context, f *verification.Flow, i *identity.Identity, a *identity.VerifiableAddress, transientPayload json.RawMessage) (err error) {
	...
	return s.deps.CodeSender().SendVerificationCodeTo(ctx, f, i, rawCode, code, transientPayload)
}
