...
func (s *Strategy) verificationHandleFormSubmission(w http.ResponseWriter, r *http.Request, f *verification.Flow, body *updateVerificationFlowWithCodeMethod) error {
	...
	if len(body.Phone) != 0 && !s.deps.Config().SelfServiceCodeStrategy(r.Context()).ExternalSMSVerify.Enabled {
		return s.handleVerificationError(w, r, f, body, errors.New("External SMS verification service is disabled"))
	}
...
func (s *Strategy) SendVerification(ctx context.Context, f *verification.Flow, i *identity.Identity, a *identity.VerifiableAddress) (err error) {
	var rawCode string
	if s.deps.Config().SelfServiceCodeStrategy(ctx).ExternalSMSVerify.Enabled && a.Via == identity.AddressTypePhone {
		rawCode = "external"
	} else {
