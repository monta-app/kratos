...
func (s *Strategy) populateMethod(r *http.Request, c *container.Container, message func(provider string, providerId string) *text.Message) error {
	...
	case *registration.Flow:
		...
		if dup := new(identity.ErrDuplicateCredentials); errors.As(err, &dup) {
			...
			if dc, err := flow.DuplicateCredentials(lf); err == nil && dc != nil {
				...
				lf.UI.Messages.Add(text.NewInfoLoginLinkMessage(dc.DuplicateIdentifier, providerLabel, newLoginURL, dup.AvailableCredentials(), dup.AvailableOIDCProviders()))
