...
func (s *Strategy) linkCredentials(ctx context.Context, i *identity.Identity, idToken, accessToken, refreshToken, providerID string, claims *Claims) error {
	...
	if errors.Is(err, herodot.ErrNotFound) {
		var err error
		if creds, err = identity.NewCredentialsOIDC(idToken, accessToken, refreshToken, providerID, claims.Subject); err != nil {
			return err
		}
	} else if err != nil {
		return err
